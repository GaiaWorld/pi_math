...
/// aabb的查询函数的参数
pub struct AbQueryArgs<S: Scalar + RealField + Float, T: Clone + PartialOrd> {
    pub aabb: AABB<S>,
    pub result: (usize, T),
}
impl<S: Scalar + RealField + Float, T: Clone + PartialOrd> AbQueryArgs<S, T> {
    pub fn new(aabb: AABB<S>, min: T) -> AbQueryArgs<S, T> {
        AbQueryArgs {
            aabb: aabb,
            result: (0, min),
        }
